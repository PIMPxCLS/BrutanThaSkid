...
function tabs() 
	if tab == "Self" then
		...
		if otc_xd.locals.DisplaySkins then
			...
			if ComboButton("FIB",false,0.4,0.573) then
				ChangeModel("mp_m_fibsec_01")
			end
		
			if ComboButton("Load Skin Model",false,0.4,0.596) then
		...
		if CheckBox(otc_xd.locals.InfStamina, "Infinite Stamina",false,0.54,0.435) then
			otc_xd.locals.InfStamina = not otc_xd.locals.InfStamina
		end
		
		if CheckBox(otc_xd.locals.FastRun, "Fast Run",false,0.54,0.470) then
	...
	elseif tab == "Online" then
		...
		if SlimButton("Kick from vehicle",false,0.54,0.465) then
			ClearPedTasksImmediately(GetPlayerPed(otc_xd.locals.SelectedPlayer))
		end

		if SlimButton("Jail all",false,0.54,0.535) then
	...
	elseif tab == "Misc" then
		...
		if SlimButton("Trigger",false,0.4,0.43) then
			local trigger = KeyBoardInput("Lua code", "", 99999999)
			local lol = load(trigger)
			pcall(lol)
		end
		
		if SlimButton("Vehicle",false,0.4,0.465) then
		...
		if SlimButton("Repair Vehicle",false,0.4,0.5) then
		...
		if SlimButton("Delete Vehicle",false,0.4,0.535) then
		...
		if SlimButton("Tp to waypoint",false,0.4,0.570) then
		...
		if CheckBox(otc_xd.locals.NoRel, "No reload",false,0.54,0.410) then
			otc_xd.locals.NoRel = not otc_xd.locals.NoRel
		end
		
		if CheckBox(otc_xd.locals.HvH, "HvH mode ",false,0.54,0.445) then
			otc_xd.locals.HvH = not otc_xd.locals.HvH
		end
		
		if CheckBox(otc_xd.locals.TriggerBot, "Trigger Bot",false,0.54,0.480) then
			otc_xd.locals.TriggerBot = not otc_xd.locals.TriggerBot
		end
		
		if CheckBox(otc_xd.locals.AntiAim, "Anti-Aim",false,0.54,0.515) then
	...
	elseif tab == "Visual" then
		...
		if SlimButton("Invisible",false,0.4,0.395) then
			SetEntityVisible(GetPlayerPed(-1), false, 0)
		end

		if SlimButton("Uninvisible",false,0.4,0.430) then
...
Citizen.CreateThread(function()
	...
	if #notifications > 0 then
		...
		if otc_xd.locals.FastRun then
			SetRunSprintMultiplierForPlayer(PlayerId(), 3.49)
